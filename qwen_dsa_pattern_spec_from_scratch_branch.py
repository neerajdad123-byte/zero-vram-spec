...
class PythonSyntaxProposer:
    ...
    def find_rule(
        ...
        if token is None and (text.endswith("```python\n") or text.endswith("``` python\n")):
            if re.search(r"(linked|tree|heap|stack|queue|bst|node|graph|sort|search)", text, re.I):
...
def run_speculative(
    ...
    # Change 4: EV scheduler — track rolling acceptance and decode cost
    ev_window = []  # list of (accepted, proposed, decode_ms)
    ev_window_size = 10
    greedy_ms_per_token = 35.0  # approximate, will calibrate
    ...
    while len(gen) < target:
        ...
        # Change 4: EV scheduler — only draft if expected value is positive
        if max_draft > 0 and len(ev_window) >= 3:
            acc_rate = sum(a for a, p, d in ev_window) / max(1, sum(p for a, p, d in ev_window))
            avg_decode_ms = sum(d for a, p, d in ev_window) / len(ev_window) * 1000
            expected_saved_ms = acc_rate * max_draft * greedy_ms_per_token - avg_decode_ms
            if expected_saved_ms <= 0:
                max_draft = 0  # EV negative — skip drafting this round
        ...
        # Change 4: Update EV window
        if draft:
            ev_window.append((accepted_draft, len(draft), decode_dt))
            if len(ev_window) > ev_window_size:
                ev_window.pop(0)
